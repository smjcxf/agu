...
_tdx_client = None
_tdx_call_count = 0
_TDX_RESET_INTERVAL = 50  # 每50次mootdx调用后重置客户端，防止socket泄漏导致进程崩溃
_tdx_lock = __import__('threading').Lock()  # 多线程安全的重置锁

def _tdx_reset():
    """重置mootdx客户端，释放累积的socket句柄"""
    global _tdx_client, _tdx_call_count
    with _tdx_lock:
        if _tdx_client is not None:
            try:
                # 尝试关闭底层socket连接
                _tdx_client.client.shutdown(2)
                _tdx_client.client.close()
            except Exception:
                pass
            _tdx_client = None
        _tdx_call_count = 0
...
def fetch_hk_daily(code, bars=None):
    ...
    try:
        import efinance as ef
        # efinance港股代码需要 hk 前缀（如 hk00700）
        df = ef.stock.get_quote_history(f"hk{code}")
        if df is not None and len(df) >= 60:
            result = pd.DataFrame({
