...
def main():
    ...
    if watch_data and watch_data.get('triple_signals'):
        ...
        if today.weekday() < 5:
            ...
            if today_str not in snapshots:
                snapshots[today_str] = []
            existing_codes = {s['code'] for s in snapshots[today_str]}
    ...
    # 合并已有数据（跳过周末，防止复活旧假数据）
    if os.path.exists(OUTPUT_FILE):
        with open(OUTPUT_FILE, 'r', encoding='utf-8') as f:
            existing = json.load(f)
        for date, data in existing.items():
            dt = datetime.strptime(date, '%Y-%m-%d')
            if dt.weekday() < 5 and date not in snapshots:
                snapshots[date] = data
        merged = sum(1 for d in existing if datetime.strptime(d, '%Y-%m-%d').weekday() < 5)
        print(f"加载已有数据: {len(existing)}个日期（过滤后{merged}个）")
